function ClassElementCell(elementName, responseText) {
	...
	joint.shapes.custom[elementName + "View"] = joint.dia.ElementView.extend({
		...
		updateBox : function() {
			...
			this.$box.find('.HtmlClassNameText')[0].innerHTML = this.model
					.get('model').name
					+ className;
			...
			if (this.model.get('model').properties.length > 0) {
				...
				this.$box.find('.HtmlClassAttributeText')[0].innerHTML = text;
			}
			...
			if (this.model.get('model').operations.length > 0) {
				...
				this.$box.find('.HtmlClassMethodText')[0].innerHTML = text;
			}
			this.$box.css({
				width : bbox.width,
				height : bbox.height,
