...
import maineditor.diagram.edit.commands.PathCreateCommand;
import maineditor.diagram.edit.commands.PathReorientCommand;
 ...
 * @generated
 */
public class LevelItemSemanticEditPolicy extends MaineditorBaseItemSemanticEditPolicy {
	...
	*/
	protected Command getDestroyElementCommand(DestroyElementRequest req) {
		...
		for (Iterator<?> it = view.getTargetEdges().iterator(); it.hasNext();) {
			...
			if (MaineditorVisualIDRegistry.getVisualID(incomingLink) == PathEditPart.VISUAL_ID) {
		...
		for (Iterator<?> it = view.getSourceEdges().iterator(); it.hasNext();) {
			...
			if (MaineditorVisualIDRegistry.getVisualID(outgoingLink) == PathEditPart.VISUAL_ID) {
	...
	*/
	private void addDestroyChildNodesCommand(ICompositeCommand cmd) {
		...
		for (Iterator<?> nit = view.getChildren().iterator(); nit.hasNext();) {
			...
			switch (MaineditorVisualIDRegistry.getVisualID(node)) {
			case LevelLevelObjectivesCompartmentEditPart.VISUAL_ID:
				for (Iterator<?> cit = node.getChildren().iterator(); cit.hasNext();) {
					Node cnode = (Node) cit.next();
					switch (MaineditorVisualIDRegistry.getVisualID(cnode)) {
	...
	protected Command getStartCreateRelationshipCommand(CreateRelationshipRequest req) {
		if (MaineditorElementTypes.Path_4001 == req.getElementType()) {
