...
import gamifiedmodelling.diagram.edit.commands.PathCreateCommand;
import gamifiedmodelling.diagram.edit.commands.PathReorientCommand;
 ...
 * @generated
 */
public class LevelItemSemanticEditPolicy extends GamifiedmodellingBaseItemSemanticEditPolicy {
	...
	*/
	protected Command getDestroyElementCommand(DestroyElementRequest req) {
		...
		for (Iterator<?> it = view.getTargetEdges().iterator(); it.hasNext();) {
			...
			if (GamifiedmodellingVisualIDRegistry.getVisualID(incomingLink) == PathEditPart.VISUAL_ID) {
		...
		for (Iterator<?> it = view.getSourceEdges().iterator(); it.hasNext();) {
			...
			if (GamifiedmodellingVisualIDRegistry.getVisualID(outgoingLink) == PathEditPart.VISUAL_ID) {
	...
	*/
	private void addDestroyChildNodesCommand(ICompositeCommand cmd) {
		...
		for (Iterator<?> nit = view.getChildren().iterator(); nit.hasNext();) {
			...
			switch (GamifiedmodellingVisualIDRegistry.getVisualID(node)) {
			case LevelLevelObjectivesCompartmentEditPart.VISUAL_ID:
				for (Iterator<?> cit = node.getChildren().iterator(); cit.hasNext();) {
					Node cnode = (Node) cit.next();
					switch (GamifiedmodellingVisualIDRegistry.getVisualID(cnode)) {
	...
	protected Command getStartCreateRelationshipCommand(CreateRelationshipRequest req) {
		if (GamifiedmodellingElementTypes.Path_4001 == req.getElementType()) {
